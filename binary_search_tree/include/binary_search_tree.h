...
public:
    ...
    virtual void call_node_constructor(node* raw_space, Tkey const &key, Tvalue const &value) const
    {
        allocator::construct(raw_space, key, value);
    }

    virtual void call_node_constructor(node* raw_space, Tkey const &key, Tvalue &&value) const
    {
    ...
    public:
        ...
        [[nodiscard]] inline bool is_state_initialized() const noexcept
        {
            return is_init_;
        }
    ...
    public:
        explicit iterator_data(): is_init_(false){}

        //common constructor
        explicit iterator_data(const unsigned int depth, Tkey const &key, Tvalue const &value):
            is_init_(true),
            depth(depth),
            key(key),
            value(value)
        ...
        // constructor with move value
        explicit iterator_data(const unsigned int depth, Tkey const &key, Tvalue &&value):
            is_init_(true),
            depth(depth),
            key(key),
...
public:
    //allocation constructor for iterator
    virtual void call_iterator_data_construct(iterator_data* raw_space, node* subtree_root, unsigned int depth) const
    {
...
protected:
    ...
    // getting paths
    void find_prefix_path(const unsigned int depth, std::queue<iterator_data*> &_pat, node* ptr) const
    {
        if (ptr == nullptr) {
            return;
        }
    ...
    void find_infix_path(const unsigned int depth, std::queue<typename binary_search_tree<Tkey, Tvalue>::iterator_data*> &_pat, binary_search_tree<Tkey, Tvalue>::node * ptr) const
    {
        if (ptr == nullptr) {
            return;
        }
    ...
    void find_postfix_path(const unsigned int depth, std::queue<typename binary_search_tree<Tkey, Tvalue>::iterator_data*> &pat, binary_search_tree<Tkey, Tvalue>::node * ptr) const
    {
        if (ptr == nullptr) {
            return;
        }
        if (ptr -> left_subtree != nullptr) {
            find_postfix_path(depth + 1, pat, ptr->left_subtree);
        }
        if (ptr -> right_subtree != nullptr) {
            find_postfix_path(depth + 1, pat, ptr->right_subtree);
        }
        pat.push(this->create_iterator_data(depth, ptr));
    }

    void clear_path(std::queue<iterator_data*> &pat) const
    {
        // deleting all nodes in q
        while (!pat.empty()) {
            iterator_data* tmp = pat.front();
            pat.pop();
...
public:
    ...
    std::queue<iterator_data*> copy_path(std::queue<iterator_data*> pat)
    {
        ...
        while (!pat.empty()) {
            iterator_data *tmp = pat.front();
            pat.pop();
            new_pat.push(create_iterator_data(tmp->depth, tmp->key, tmp->value));
        }
        return new_pat;
    }

    void iterators_copy(iterator_data *& ptr, iterator_data *& s_ptr,
    binary_search_tree const *& tree, binary_search_tree const *& s_tree,
...
public:
    ...
    public:
        ...
        explicit prefix_iterator(const binary_search_tree* tree, node *subtree_root);
    ...
    private:
        const binary_search_tree* _tree;
    ...
    public:
        ...
        explicit prefix_const_iterator(const binary_search_tree *tree, node *subtree_root);
    ...
    class prefix_reverse_iterator: protected allocator_guardant
    {

    public:

        explicit prefix_reverse_iterator(const binary_search_tree *tree, node *subtree_root);
    ...
    public:
        ...
        explicit prefix_const_reverse_iterator(const binary_search_tree* tree, node *subtree_root);
    ...
    public:
        ...
        prefix_const_reverse_iterator  operator++(int not_used);
        iterator_data const*           operator* ()             const;
    ...
    public:
        ...
        explicit infix_iterator(const binary_search_tree* tree, node* subtree_root);
    ...
    public:
        ...
        explicit infix_reverse_iterator(const binary_search_tree *tree, node *subtree_root);
    ...
    public:
        ...
        {
            if (!_pat.empty()) _tree->clear_path(_pat);
...
protected:
    ...
    public:
        ...
        explicit template_method_basics(binary_search_tree *tree);
    ...
    class insertion_template_method final: public template_method_basics, public allocator_guardant
    {
    public:
        ...
        void set_insertion_strategy(insertion_of_existent_key_attempt_strategy insertion_strategy) noexcept;
    ...
    public:
        ...
        void set_disposal_strategy(disposal_of_nonexistent_key_attempt_strategy disposal_strategy) noexcept;
...
public:
    ...
    explicit binary_search_tree(
        ...
        insertion_of_existent_key_attempt_strategy   insertion_strategy = insertion_of_existent_key_attempt_strategy::throw_an_exception,
        disposal_of_nonexistent_key_attempt_strategy disposal_strategy  = disposal_of_nonexistent_key_attempt_strategy::throw_an_exception);
...
public:
    void set_insertion_strategy(insertion_of_existent_key_attempt_strategy insertion_strategy)  noexcept;
    void set_removal_strategy  (disposal_of_nonexistent_key_attempt_strategy disposal_strategy) noexcept;
...
public:
    ...
    void small_left_rotation  (node *&subtree_root, bool validate = true) const;
    void small_right_rotation (node *&subtree_root, bool validate = true) const;

    void big_left_rotation    (node *&subtree_root, bool validate = true) const;
    void big_right_rotation   (node *&subtree_root, bool validate = true) const;

    void double_left_rotation (node *&subtree_root, bool at_grandparent_first, bool validate = true) const;
    void double_right_rotation(node *&subtree_root, bool at_grandparent_first, bool validate = true) const;
...
template<typename tkey, typename tvalue>
bool binary_search_tree<tkey, tvalue>::prefix_iterator::operator==(prefix_iterator const &other) const noexcept
{
...
template<typename tkey, typename tvalue>
bool binary_search_tree<tkey, tvalue>::prefix_const_iterator::operator==(prefix_const_iterator const &other) const noexcept
{
    return _ptr == other._ptr;
}
...
binary_search_tree<tkey, tvalue>::prefix_reverse_iterator::prefix_reverse_iterator(
    const binary_search_tree* tree, node *subtree_root) : _tree(tree)
{
    if (subtree_root == nullptr) {
        _ptr = nullptr;
        return;
...
template<typename tkey, typename tvalue>
typename binary_search_tree<tkey, tvalue>::prefix_const_reverse_iterator binary_search_tree<tkey, tvalue>::prefix_const_reverse_iterator::operator++(int not_used)
{
...
binary_search_tree<tkey, tvalue>::infix_const_iterator::infix_const_iterator(
    const binary_search_tree* tree, node *subtree_root) :
    _tree(tree)
{
    if (subtree_root == nullptr) {
        _ptr = nullptr;
...
template<typename tkey, typename tvalue>
void binary_search_tree<tkey, tvalue>::small_right_rotation(node *&subtree_root, bool validate) const
{
...
void binary_search_tree<tkey, tvalue>::double_left_rotation(
    binary_search_tree<tkey, tvalue>::node *&subtree_root,
    const bool at_grandparent_first,
    const bool validate) const
{
