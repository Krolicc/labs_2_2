...
node_BST *createnode_BST(char* key, otd* otdd) {
    node_BST *newnode_BST = malloc(sizeof(node_BST));
    if (newnode_BST == NULL) return NULL;
...
node_BST *insert_BST(node_BST *root, char* key, otd* value) {
    if (root == NULL) return createnode_BST(key, value);
...
otd *search_BST(const node_BST *root, char* key) {
