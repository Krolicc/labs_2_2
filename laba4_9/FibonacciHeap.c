...
fib_heap* make_fib_heap() {
    fib_heap* heap = malloc(sizeof(fib_heap));
...
fib_heap_node* make_fib_node(application* data) {
    fib_heap_node* node = malloc(sizeof(fib_heap_node));
...
enum answer fib_heap_insert(void* temp_heap, application* data) {
    fib_heap* heap = (fib_heap*)temp_heap;

    fib_heap_node* node = make_fib_node(data);
    if (node == NULL) return ERROR_MEMORY;

    if (heap->max == NULL) heap->max = node;
    else {
        node->left = heap->max->left;
        node->right = heap->max;
        heap->max->left->right = node;
        heap->max->left = node;
        if ((node->data->prior > heap->max->data->prior) || 
        (node->data->prior == heap->max->data->prior && strcmp(node->data->time_app, heap->max->data->time_app) < 0)) {
            heap->max = node;
        }
    }
    heap->num_nodes++;
...
void fib_heap_merge_without_destr(void* temp_H1, void* temp_H2) {
    fib_heap* H1 = (fib_heap*)temp_H1;
    fib_heap* H2 = (fib_heap*)temp_H2;
    if (H1->max == NULL) H1->max = H2->max;
    else if (H2->max != NULL) {
        ...
        if (H2->max->data->prior > H1->max->data->prior ||
            (H2->max->data->prior == H1->max->data->prior && strcmp(H2->max->data->time_app, H1->max->data->time_app) < 0)) {
            H1->max = H2->max;
        }
    }
...
fib_heap* merge_fib_heaps_destr(fib_heap* heap1, fib_heap* heap2) {
    ...
    if (heap1->max == NULL) new_heap->max = heap2->max;
        ...
        if (heap2->max->data->prior > heap1->max->data->prior ||
            (heap2->max->data->prior == heap1->max->data->prior && strcmp(heap2->max->data->time_app, heap1->max->data->time_app) < 0)) {
            new_heap->max = heap2->max;
        } else new_heap->max = heap1->max;
    } else new_heap->max = heap1->max;
...
enum answer consolidate(fib_heap* heap) {
    ...
    const int MAXDEGREE = log2(heap->num_nodes);
    fib_heap_node** A = malloc(sizeof(fib_heap_node*) * MAXDEGREE); // Массив для хранения узлов по их степеням
    if (A == NULL) return ERROR_MEMORY;
    ...
    while (current != heap->max) {
        ...
        while (A[degree] != NULL) {
            ...
            if (conflict->data->prior > current->data->prior) {
                addTo = conflict;
                adding = current;
            } else {
                addTo = current;
    ...
    for (int i = 0; i < MAXDEGREE; i++) {
        if (A[i] != NULL && A[i]->data->prior > max->data->prior  ) {
            max = A[i];
        }
    }
...
application* find_max_fib(const fib_heap* heap) {
    if (heap == NULL) return NULL;

    return heap->max->data;
}

application* extract_max_fib(void* temp_heap) {
    fib_heap* heap = (fib_heap*)temp_heap;
    if (heap == NULL || heap->max == NULL) return NULL; // Куча пуста, возвращаем NULL

    fib_heap_node* maxNode = heap->max;
    application* maxApp = maxNode->data;
    // Удаление максимального узла из корневого списка
    if (maxNode->right == maxNode) heap->max = NULL; // Был всего один узел, делаем кучу пустой
    else {
...
void free_node_fib_helper(fib_heap_node *node, int isFull) {
    ...
    if (child != NULL) {
        const fib_heap_node* temp = child;
        do {
            fib_heap_node* next = child->right;
...
void free_fib_heap(void* temp_heap, int isFull) {
    fib_heap* heap = (fib_heap*)temp_heap;
    if (heap == NULL) return;
